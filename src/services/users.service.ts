...
class UserService {
    ...
    public async createUser(userData: CreateUserDto): Promise<User> {
        ...
        const findUserNickname: User = await this.users.findOne({ nickname: userData.nickname });
        if (findUserNickname)
            throw new HttpException(409, `You're email ${userData.nickname} already exists`);
